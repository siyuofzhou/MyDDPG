...
class JointNetWork:
    ...
    def __init__(self,state_dim,action_dim):
        ...
        self.__soft_update_rate = 0.001 #soft更新target中的变量的变换比例
        self.__state_dim = state_dim
        self.__action_dim = action_dim
        self.__gama = 0.99 #折扣回馈法中的系数
        self.__policy_learn_rate = 1e-6
        self.__Q_learn_rate = 1e-6
        ...
        self.__save_path = '.\\model_v2\\model'
        self.__save_dir = '.\\model_v2'
        self.__save_file = '.\\model_v2\\checkpoint'
    ...
    def __dense(self,input,uint,collections=None,active=True,name=None,reg=None):
        with tf.variable_scope(name):
            w = tf.get_variable('w',shape=(input.shape[1],uint),initializer=tf.truncated_normal_initializer(),collections=collections,regularizer=reg)
            b = tf.get_variable('b',shape=(uint,),initializer=tf.truncated_normal_initializer(),collections=collections,regularizer=reg)
    ...
    def __network(self,input,netname,online=True,reg=None):
       
        i = input
        policyLayers = [self.__state_dim,64,128,128,128,64] 
        QLayers = [self.__state_dim+self.__action_dim,64,128,128,128,64] 
        ...
        with tf.variable_scope(netname,reuse=tf.AUTO_REUSE):
            with tf.variable_scope('policy',reuse=tf.AUTO_REUSE):
                for index,layer in enumerate(policyLayers):
                    #print('plicy_layer:',index,layer)
                    i = self.__dense(i,layer,collections=collections,name='dense_'+str(index),reg=reg)
                    
                act = self.__dense(i,self.__action_dim,collections=collections,active=False,name='dense_action',reg=reg)
                with tf.variable_scope('act'):
                    act = tf.divide(act,tf.expand_dims(tf.reduce_max(tf.abs(act),axis=1)+1e-8,axis=1))
            ...
            with tf.variable_scope('Q',reuse=tf.AUTO_REUSE):
                ...
                for index,layer in enumerate(QLayers):
                    #print('Q_layer:',index,layer)
                    i = self.__dense(i,layer,collections=collections,name='dense_'+str(index),reg=reg)
                    
                i = self.__dense(i,1,collections=collections,active=False,name='Q',reg=reg)
    ...
    def __PolicyLoss(self,y_true,reg=None,name=None):
        with tf.variable_scope(name):
            return -1.0*tf.reduce_mean(y_true)+reg
    ...
    def __build(self):
        reg_l2 = tf.contrib.layers.l2_regularizer(1e-5)
        
        self.target_Q = self.__network(self.target_state_input,'target',online=False)
        self.online_action,self.online_Q = self.__network(self.online_state_input,'online',reg=reg_l2)
        ...
        with tf.variable_scope('Loss'):
            ...
            self.policyLoss = self.__PolicyLoss(self.online_Q,reg = policy_loss_l2,name='policy')
        ...
        scalar_reg = tf.summary.scalar('reg',Q_loss_l2*0.05+policy_loss_l2)
        ...
        for i,(g,v) in enumerate(grads):
            if g is not None:
                grads[i] = (tf.clip_by_value(g,-1.0,1.0),v)
